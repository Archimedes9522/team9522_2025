...
public class Vision extends SubsystemBase {
  ...
  private static final List<Integer> REEF_TAGS = Arrays.asList(6, 7, 8, 9, 10, 11, 17, 18, 19, 20, 21, 22); // List of
                                                                                                            // allowed
                                                                                                            // tag IDs
  ...
  private Pose3d getClosestTagPose(
    ...
    return Arrays.stream(input.tagIds)
        ...
        .filter(
            tagPose -> {
              // Check if the tag is within the 5 feet distance from the robot's current
              // position
              double distance = tagPose
                  .getTranslation()
                  .getDistance(
                      input.poseObservations.length > 0
                          ? input.poseObservations[0]
                              .pose()
    ...
    for (int cameraIndex = 0; cameraIndex < io.length; cameraIndex++) {
      ...
      if (!inputs[cameraIndex].isLocked) {
        if (cameraIndex == 1) {
          ...
          inputs[cameraIndex].closestTag = getClosestTagPose(inputs[cameraIndex], CORAL_STATION_TAGS, 10);
        }
      }
