...
public class AlignToLeftOfTagCommand extends Command {
    ...
    private final double OFFSET_FROM_TAG_METERS = 0.5; // Left offset

    public AlignToLeftOfTagCommand(DriveSubsystem drivetrain, VisionSubsystem vision) {
    ...
    @Override
    public void execute() {
        var target = vision.getRightCameraTarget(); // Use right camera to align to left

        if (target != null) {
            // Calculate target yaw - add offset to align to left of tag
            double targetYaw = target.getYaw() + 15.0; // Add offset in degrees
